...
use crate::{
    ...
    spec::{arg_size, by_name, OpSpec, ARG_DIR, ARG_IND, ARG_REG, DESCRIPTION_LENGTH, PLAYER_MAX_SIZE, PROG_NAME_LENGTH, REG_NUMBER},
};
...
pub fn parse(source: &str) -> Result<Program, AsmError> {
    ...
    for source_line in lines {
        ...
        if code.starts_with('.') {
            if code_started {
                return Err(AsmError::line(line_no, "header directives must appear before labels and instructions"));
            }
            if code.starts_with(".name") {
                ...
                if value.as_bytes().len() > PROG_NAME_LENGTH {
                    return Err(AsmError::line(line_no, format!("name exceeds {PROG_NAME_LENGTH} bytes")));
                }
                name = Some(value);
                continue;
            }
            if code.starts_with(".description") {
                ...
                if value.as_bytes().len() > DESCRIPTION_LENGTH {
                    return Err(AsmError::line(line_no, format!("description exceeds {DESCRIPTION_LENGTH} bytes")));
                }
                description = Some(value);
                continue;
            }
            return Err(AsmError::line(line_no, format!("unknown directive in '{code}'")));
        }

        if name.is_none() || description.is_none() {
            return Err(AsmError::line(line_no, ".name and .description must appear before code"));
        }
        code_started = true;

        let mut rest = code;
        loop {
            let Some((label, tail)) = take_leading_label(rest) else { break; };
            if labels.insert(label.to_owned(), offset).is_some() {
                return Err(AsmError::line(line_no, format!("duplicate label '{label}'")));
            }
        ...
        let op = by_name(mnemonic).ok_or_else(|| AsmError::line(line_no, format!("unknown instruction '{mnemonic}'")))?;
        let raw_args = split_arguments(arg_text, line_no)?;
        if raw_args.len() != op.allowed.len() {
            return Err(AsmError::line(line_no, format!("{} expects {} argument(s), got {}", op.name, op.allowed.len(), raw_args.len())));
        }
        ...
        for (idx, raw) in raw_args.iter().enumerate() {
            ...
            if op.allowed[idx] & kind == 0 {
                return Err(AsmError::line(line_no, format!("invalid argument {} type for {}", idx + 1, op.name)));
            }
            size += arg_size(kind, op.has_idx);
            args.push(arg);
        }

        instructions.push(Instruction { op, args, offset, size, line: line_no });
        offset = offset.checked_add(size).ok_or_else(|| AsmError::line(line_no, "program size overflow"))?;
        if offset > PLAYER_MAX_SIZE {
            return Err(AsmError::line(line_no, format!("program exceeds maximum size of {PLAYER_MAX_SIZE} bytes")));
        }
    }
    ...
    Ok(Program { name, description, instructions, labels, code_size: offset })
}
...
fn parse_quoted_directive(code: &str, directive: &str, line: usize) -> Result<String, AsmError> {
    ...
    if !rest.starts_with('"') {
        return Err(AsmError::line(line, format!("{directive} requires a quoted string")));
    }
    let tail = &rest[1..];
    let Some(end) = tail.find('"') else {
        return Err(AsmError::line(line, format!("unterminated string in {directive}")));
    };
    if !tail[end + 1..].trim().is_empty() {
        return Err(AsmError::line(line, format!("unexpected text after {directive} string")));
    }
...
fn parse_argument(raw: &str, line: usize) -> Result<Argument, AsmError> {
    ...
    if let Some(rest) = raw.strip_prefix('r') {
        if !rest.is_empty() && rest.bytes().all(|c| c.is_ascii_digit()) {
            let reg = rest.parse::<u16>().map_err(|_| AsmError::line(line, format!("invalid register '{raw}'")))?;
            if reg == 0 || reg > REG_NUMBER as u16 {
                return Err(AsmError::line(line, format!("register out of range: '{raw}'")));
            }
